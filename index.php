...
if ($debug == 1) {
	...
	echo "<br>bo direct getfromurl=" . $bo;
	echo "<br>sig=" . $sig;
	...
	echo "<br>staff_status=" . $staff_status;
} else {
...
if ($sig != "" || $uid != "") {
	...
	if (($uid != "" && $smartcard_status == true) || $sig != "") {
		...
		// redirect to which path VCF or e-profile or weblink1

		if ($debug == 1) {
			echo "<br>bo=" . $bo;
			echo "<br>bizcard_option=" . $bizcard_option;
			echo "<br>staffstatus=" . $staff_status;
		}

			if ($qrtype == "" && $genaw != 1 && $gengw != 1 && $mobilesite != 1 && $geniosaos != 1) {
					if ($bizcard_option == 1 || $bo == 1) {
						include_once("profile.php");
						//$str=$domain."/Touchless/Profile.php?sig=".$sig."#resume"; 
					} else if ($bizcard_option == 2 || $bo == 2) {
						header("Location: " . $web_link);
					}else if ($bizcard_option == 0 || $bo == 0) {
